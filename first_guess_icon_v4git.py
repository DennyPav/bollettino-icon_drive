...
RUN_HOURS = ['00', '12']
VARIABLES = ['T_2M', 'RELHUM', 'TOT_PREC', 'CLCT', 'CLCL', 'CLCM', 'CLCH', 'PMSL', 'U_10M', 'V_10M', 'VMAX_10M', 'LPI', 'CAPE_ML', 'CAPE_CON', 'UH_MAX', 'HSURF']
CAPOLUOGHI = {
...
def weather_data(data):
    ...
    for city, (lat_c, lon_c, alt_c) in CAPOLUOGHI.items():
        ...
        clcl = extract_variable(clcl_raw, lat_idx, lon_idx)
        clcm = extract_variable(clcm_raw, lat_idx, lon_idx)
        clch = extract_variable(clch_raw, lat_idx, lon_idx)
        vmax10 = extract_variable(vmax10_raw, lat_idx, lon_idx)
        lpi = extract_variable(lpi_raw, lat_idx, lon_idx)
        cape = extract_variable(cape_raw, lat_idx, lon_idx)
        uh = extract_variable(uh_raw, lat_idx, lon_idx)
        ...
        capoluoghi_dati[city] = {
            ...
            'vmax_10m': np.round(vmax10, 1),
            'lpi': np.round(lpi, 2),
            'cape_ml': np.round(cape, 1),
            'uh_max': np.round(uh, 1),
...
def add_daily_summary(pdf_obj, daily_summary_data, icons_path):
    ...
    icon_filename = daily_summary_data.get('Icona_Mappata', 'sereno_g.png')
...
def map_text_to_icon(text, is_day, wind_speed, city_name):
    if city_name.startswith("Mar_"):
        if wind_speed < 10: return "mare_1.png"
...
def generate_weather_bulletin(city_name, capoluoghi_dati, run_datetime_utc, output_dir, icons_path, font_path, logo_path=None):
    ...
    df = pd.DataFrame({
        ...
        'clcl': truncated_dati.get('clcl', np.zeros_like(truncated_dati['t2m'])),
        'clcm': truncated_dati.get('clcm', np.zeros_like(truncated_dati['t2m'])),
        'clch': truncated_dati.get('clch', np.zeros_like(truncated_dati['t2m'])),
        'vmax_10m': truncated_dati.get('vmax_10m', np.zeros_like(truncated_dati['t2m'])),
        'lpi': truncated_dati.get('lpi', np.zeros_like(truncated_dati['t2m'])),
        'cape_ml': truncated_dati.get('cape_ml', np.zeros_like(truncated_dati['t2m'])),
        'uh_max': truncated_dati.get('uh_max', np.zeros_like(truncated_dati['t2m'])),
    })
    ...
    for day, day_group in df.groupby('Giorno_Locale'):
        ...
        season = get_season_precise(day)
        recs = []
        for _, r in day_group.iterrows():
            wtxt = classify_weather_hourly(
                t2m=r['Temperatura (°C)'], rh2m=r['Umidità (%)'],
                clct=r['Nuvolosità (%)'], clcl=r['clcl'], clcm=r['clcm'], clch=r['clch'],
                tp_rate=r['Precipitazione (mm)'], wind_kmh=r['vmax_10m'],
                lpi=r['lpi'], cape=r['cape_ml'], uh=r['uh_max'],
                season=season, season_thresh=SEASON_THRESHOLDS[season]
            )
            offset_h = get_local_time_offset(r['Data/Ora'].to_pydatetime())
            loc_time = r['Data/Ora'] + timedelta(hours=offset_h)
            recs.append({"h": loc_time.hour, "w": wtxt})

        daily_wtxt = classify_daily_weather(
            recs=recs,
            clct_avg=day_group['Nuvolosità (%)'].mean(),
            clcl_avg=day_group['clcl'].mean(),
            clcm_avg=day_group['clcm'].mean(),
            clch_avg=day_group['clch'].mean(),
            tp_tot=day_group['Precipitazione (mm)'].sum(),
            season=season,
            thresh=SEASON_THRESHOLDS[season]
        )

        giorno_anno = ref_time_local.timetuple().tm_yday
        ...
        daily_summaries[day] = {
            ...
            'Ora Riferimento': ref_time_local.to_pydatetime(),
            'City Name': city_name,
            'Icona_Mappata': icona_mapped
        }
